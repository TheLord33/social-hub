...
import { getToken, refreshTwitterIfNeeded, refreshTikTokIfNeeded, refreshRedditIfNeeded } from "@/lib/tokens";
...
async function postToReddit(
  ...
  if (!title.trim()) return { platform: "reddit", success: false, error: "Post title is required for Reddit." };

  // text=self post, image/video=link post pointing at media URL
  const kind = mediaType === "text" ? "self" : "link";
  const bodyParams: Record<string, string> =
    kind === "self" ? { text: content } : { url: mediaUrl ?? content };
  ...
  return { platform: "reddit", success: true, postId: data.json?.data?.id, postUrl };
}
...
export async function POST(request: NextRequest) {
  ...
  return Response.json({ results: out }, { status: out.some((r) => r.success) ? 200 : 422 });
}
